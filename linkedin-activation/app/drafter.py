...
from app import db
from app.config import DRAFT_BATCH_LIMIT, DRAFT_DAILY_LIMIT, OPENAI_API_KEY
...
"""
    ...
    db.update_outreach(outreach_id, {"draft_message": text})
...
def draft_all_detected(supabase_client: Client, limit: int = DRAFT_BATCH_LIMIT) -> int:
    """Draft detected outreach rows within the batch and daily review caps."""
    posted_today = db.get_outreach_slack_posted_today_count()
    remaining = max(0, DRAFT_DAILY_LIMIT - posted_today)
    if remaining <= 0:
        logger.info("[outreach:draft] Daily review limit reached. Skipping.")
        return 0

    fetch_limit = min(limit, DRAFT_BATCH_LIMIT, remaining)
    rows = db.get_outreach_by_status("detected", limit=fetch_limit)
    count = 0
    for row in rows:
        try:
